...
#        ggplot(data = t, aes(x=RA, y=V_A)) + geom_line(aes(col = M)) +scale_x_continuous(limits = c(0,200)) + scale_y_continuous(limits = c(0,500))
        ggplot(data = t, aes(x=RA, y=V_A)) + geom_line(aes(col = M)) + scale_x_continuous(limits = c(0,200)) + scale_y_continuous(limits = c(0,3000)) + theme(legend.title = element_blank() ) + xlab("Reference + Alternate allele count") + ylab("Variance of alternate allele count") + coord_fixed(ratio = 200/3000) + theme(legend.position="none") + theme(axis.text.x = element_text(size = 30), axis.text.y = element_text(size = 30), axis.title.y = element_text(size = 25), axis.title.x = element_text(size = 25))
        ggsave("./WEx_CEU_Indels_ggplot.pdf")
        ggsave("./WEx_CEU_Indels_ggplot.png")
	write.csv(t, file = "./variance_file.csv", row.names = F)
        return(t)
