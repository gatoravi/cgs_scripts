...
plotFits_SNPs_3 <- function(ipFile1, ipFile2, ipFile3, opFile) 
{
        require("ggplot2")
	...
	#require(ggplot2
	source("~/Scripts/RScripts/var_bin.R")
	source("~/Scripts/RScripts/var_poisson.R")
	source("~/Scripts/RScripts/betaBinomial.R")
	source("~/Scripts/RScripts/var_betbin.R")
	...
	print(ab[2])
	class(ab)
	plot(a1$Group.1, a1$x, type = "l", xlim = c(0, 200), ylim = c(0, 500), xlab = "Ref+Alt Count", ylab = "Var of Alt Count", main = opFile)
...
#        ggplot(data = t, aes(x=RA, y=V_A)) + geom_line(aes(col = M)) +scale_x_continuous(limits = c(0,200)) + scale_y_continuous(limits = c(0,500))
        ggplot(data = t, aes(x=RA, y=V_A)) + geom_line(aes(col = M)) + scale_x_continuous(limits = c(0,200)) + scale_y_continuous(limits = c(0,500)) + theme(legend.title = element_blank() ) + xlab("Reference + Alternate allele count") + ylab("Variance of alternate allele count") + theme(axis.text.x = element_text(size = 25), axis.text.y = element_text(size = 25), axis.title.y = element_text(size = 20), axis.title.x = element_text(size = 20),  legend.text = element_text(size = 20)) + coord_fixed(ratio=0.4) + theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank()) + theme(panel.background = element_rect(fill='white', colour='white'))
        ggsave("./WEx_CEU_SNPs_ggplot.pdf")
        ggsave("./WEx_CEU_SNPs_ggplot.png")
